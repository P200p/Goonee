...
+  $('balance').textContent = data.balance;
+  $('spinResult').textContent = data.payout >= 0 ? 'ชนะ ' + data.payout : 'เสีย ' + Math.abs(data.payout);
...
+  if (idx<0) return alert('คำขอไม่อยู่ในรายการ');
...
+$('clearHistory').addEventListener('click', ()=>{ if (confirm('ล้างประวัติ?')) clearHistory(); });
+
